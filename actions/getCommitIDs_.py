...
#   eclipse.jdt.core
metadata["eclipse.jdt.core"] = {}
metadata["eclipse.jdt.core"]["pathRepositoryFile"] = r"C:\Users\login\data\workspace\MLTool\datasets\eclipse.jdt.core\repositoryFile"
metadata["eclipse.jdt.core"]["pathRepositoryMethod"] = r"C:\Users\login\data\workspace\MLTool\datasets\eclipse.jdt.core\repositoryMethod"
metadata["eclipse.jdt.core"]["releaseIDs"] = [
    ["","a7fce940d379c2e5e244d9ddaf1acb77c5df6fe5"],

    None,
    ["d0dd6c20e4958b2e8f8c4f7f60f4a15fff6ca500","ba370a9c7ffb041448d1d6f1b3ed0bcf4b2f36f5"],
    ["c5f94c1be0604760b768518fd2d7014d6ae18052","7c3c5b149a5b757dafc9eee38cc2d25bd109519c"],
    ["3b4f046b547174509da38e0b0a4f7bf6125e51ec","ebc23690a9bf0afac24abe2147261af0fbe9fa10"],
...
metadata["lucene-solr"]["pathRepositoryFile"] = r"C:\Users\login\data\workspace\MLTool\datasets\lucene-solr\repositoryFile"
metadata["lucene-solr"]["pathRepositoryMethod"] = r"C:\Users\login\data\workspace\MLTool\datasets\lucene-solr\repositoryMethod"
...
metadata["poi"]["releaseIDs"] = [
    ["b3f6ea5a814f05e9e8551b965e18567de814b62b","6c234ab6c8d9b392bc93f28edf13136b3c053894"],

    None,
    ["b3f6ea5a814f05e9e8551b965e18567de814b62b","0ac1311fd0a411f6cce60e48b595844c360af466"],
    ["8ffd79b36803be29486ab6bdeff5dd02ea901927","cc1cd7f005bc2379449439427d2c0acef7cd932a"],
    ["99b9b7828cb268f62daa5b0c5639e549477b2c6f","37a74b4a8de772cde9788a6a8c2ae3c0862d31f3"],

    ["382714eccd92667fc83f70115b736c64ebff9700","449f08fc0003c64d09663715f28896a4dd010d6a"]
]
...
def identifyCommit_rbr(releaseIndex):
    ...
    if(releaseIDs[releaseIndex-2]==None):
        return None
    testCommitID          = releaseIDs[releaseIndex][1]
    trainCommitID         = releaseIDs[releaseIndex-1][1]
    previousTrainCommitID = releaseIDs[releaseIndex-2][1]
    if(releaseIndex-2<0 or previousTrainCommitID==None or trainCommitID ==None):
        return None
    test  = [ "R"+str(releaseIndex)+"_r_test", "method", "metrics", getCorrespondingCommit(testCommitID),         trainCommitID,  testCommitID, releaseIDs[len(releaseIDs)-1][1]]
    records.append(test)
    train = ["R"+str(releaseIndex)+"_r_train", "method", "metrics", getCorrespondingCommit(trainCommitID), previousTrainCommitID, trainCommitID,                       testCommitID]
    records.append(train)
def identifyCommit_cbc(commitHashes, releaseIndex, NumOfCommits):
    commitIDsTest = [None, None, None]
    commitIDsTest[1] = releaseIDs[releaseIndex][1]
    ...
    commitIDsTest[2] = releaseIDs[len(releaseIDs)-1][1]
    test  = ["R"+str(releaseIndex)+"_c"+str(NumOfCommits) + "_test"  , "method", "metrics", getCorrespondingCommit(commitIDsTest[1]), commitIDsTest[0],  commitIDsTest[1], commitIDsTest[2]]
    records.append(test)
    count = 0
    while(0 < commitHashes.index(commitIDsTest[1])-NumOfCommits-NumOfCommits*count):
        ...
        commitIDsTrain[2] = commitIDsTest[1]#commitHashes[commitHashes.index(commitIDsTest[1])                  -NumOfCommits*count]
        train = ["R"+str(releaseIndex)+"_c"+str(NumOfCommits) + "_train_" + str(count), "method", "metrics", getCorrespondingCommit(commitIDsTrain[1]), commitIDsTrain[0], commitIDsTrain[1], commitIDsTrain[2]]
        records.append(train)
        count+=1
def identifyCommit_tbt(commits, releaseIndex, numOfMonths):
    ...
    test  = ["R"+str(releaseIndex)+"_t"+str(numOfMonths) + "_test"  , "method", "metrics", getCorrespondingCommit(commitIDsTest[1]),             commitIDsTest[0], commitIDsTest[1], commitIDsTest[2]]
    records.append(test)
    ...
    while(True):
        ...
        intervals = [
            ...
            [commits[commitHashes.index(commitIDsTest[1])].committer_date - relativedelta(months = numOfMonths*1 + numOfMonths*count), commits[commitHashes.index(commitIDsTest[1])].committer_date - relativedelta(months = numOfMonths*0 + numOfMonths*count)]
        ]
        ...
        if(commitIDsTrain[0]==None or commitIDsTrain[1]==None or commitIDsTrain[2]==None):
            break
        train = ["R"+str(releaseIndex)+"_t"+str(numOfMonths) + "_train_" + str(count), "method", "metrics", getCorrespondingCommit(commitIDsTrain[1]),     commitIDsTrain[0], commitIDsTrain[1], commitIDsTrain[2]]
        records.append(train)
        count+=1
...
def main(project):
    ...
    for releaseIndex in range(len(releaseIDs)):
        print(releaseIndex)
        if(releaseIndex==0 or releaseIndex ==len(releaseIDs)-1):
            continue
        if(releaseIDs[releaseIndex]==None):
            continue
